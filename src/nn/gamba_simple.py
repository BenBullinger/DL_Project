...
class Gamba(nn.Module):
    def __init__(
        ...
        self.merge = get_mlp(input_dim=hidden_channels*2, hidden_dim=hidden_channels, output_dim=hidden_channels, mlp_depth=1, normalization=torch.nn.LayerNorm, last_relu=False)
    ...
    def forward(self, x, edge_index, batch, **kwargs):
        ...
        x_m = x_mamba[batch]
        x = self.merge(torch.cat([x, x_m[:,-1,:]], dim=1)) 
